...
class Deduplicating():
    ...
    def start_dedup(self):
        ...
        with open("config/dedup_config.yaml", "r") as config_file:
        ...
        elif args.hash_func == "xxh3":
            if HASH_BITS == 16:
                hash_func = xxh3_16hash
            else:
                hash_func = xxh3_32hash
        else:
        # 处理未知的哈希函数情况，这里可以根据实际情况进行处理
            raise ValueError(f"Unknown hash function: {args.hash_func}")
