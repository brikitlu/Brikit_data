...
from utils import get_gpt3_5_response, clean_patterns
...
class ImitateGenerator():
    ...
    def gen_ques(self, num_samples, task, language, model='gpt-3.5'):

        if self.multi_language:
            language_1 = language[0]
            language_2 = ', '.join(language[1:])
        else:
            language_1 = language if isinstance(language, str) else language[0]
        ques_ls = []
        ...
        for i in tqdm(num_samples):
            ...
            if self.multi_language:
                ...
                template_ques = self.template_ques.format(task=task, sample_str=sample_str, language_1=language_1)
            response = get_gpt3_5_response(template_ques, model)
            ques = clean(response)
            ques_ls.extend(ques)
    ...
    def gen_ans(self, model='gpt-3.5'):
        samples_gen = []
        num_fail = 0
        for ques in self.ques_ls:
            ans = get_gpt3_5_response(ques, model)
...
class AbilityExtractionGenerator():
    ...
    def ability_extraction(self, num_examples, model='gpt-3.5'):
        ...
        for i in tqdm(num_examples):
            ...
            response = get_gpt3_5_response(template_ability_extraction, model)
    ...
    def gen_sample(self, num_samples, model='gpt-3.5'):
        samples_gen_ori = []
        num_fail = 0
        for i in tqdm(range(num_samples)):
            ...
            sample = get_gpt3_5_response(template_sample_gen, model)
...
class AbilityDirectGenerator():
    ...
    def gen_ques(self, num_examples, task, num_exclude=500, model='gpt-3.5'):
        ...
        for i in tqdm(range(num_examples)):
            ...
            response = get_gpt3_5_response(ability_gen_prompt, model)
            ques = clean(response)
            ques_ls.extend(ques)
